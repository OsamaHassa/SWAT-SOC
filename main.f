...
!! current working directory - for Chris G.
      ...
      ! === OPEN SOCIO-ECONOMIC LUP LOG FILE ===
      open(soclup_log_unit, file='lup_change.log', status='replace')
      write(soclup_log_unit,*) '--- SWAT Socio-Economic Land Use Change Log ---'
      write(soclup_log_unit,*) 'Simulation started...'
...
!! process input
      ...
      call readlup
      call readsoclup             !! read the socio-economic inputs
      call readfig
      call readatmodep
      call readinpt
	if (ievent>0) then
...
!! file for Mike White to review to ensure simulation executed normally
      ...
      ! === CLOSE LOG FILE ===
      write(soclup_log_unit,*) 'Simulation complete.'
      close(soclup_log_unit)
