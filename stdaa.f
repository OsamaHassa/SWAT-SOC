      ...
      write (19,1200)
        ...
        if (xirr > 0.) then
          ...
          do j = 1, nhru
            ...
            ! SWAT-SOC
                if (aairr(j) > 1.e-4) then
                      irr_vol_mcm = aairr(j) * hru_km(j) * 0.001
                      write (26,1401) j, irn(j), aairr(j), irr_vol_mcm
                endif  
            ...
            !endif
              if (aairr(j) > 1.e-4) then
                      irr_vol_mcm = aairr(j) * hru_km(j) * 0.001
                      write (19,1401) j, irn(j), aairr(j), irr_vol_mcm
                  endif
              endif
          end do
        end if
...
3101  format (t20,'NITRATE SEPTIC = ',f12.2,' (kg/ha)',/,t20,
 ...
 ! SWAT-SOC    
 1401 format (11x,i6,9x,i4,13x,f10.3,5x,f14.4)
