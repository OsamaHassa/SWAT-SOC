...
!!    ~ ~ ~ ~ ~ ~ END SPECIFICATIONS ~ ~ ~ ~ ~ ~
      ...
      real*8 :: xx
      integer :: eof
      real*8 :: res_vol_mcm   ! Temporary variable for reservoir volume in MCM for the soci-economic inputs
      integer :: desired_lup
      
     !! === Parameters controlling trigger type and thresholds ===
      
      integer, parameter :: USE_FLOW_TRIGGER = 1
      integer, parameter :: SUB_TO_CHECK = 2
      real*8, parameter :: Q_EMERG = 7.59d0
      real*8, parameter :: Q_ALERT = 8.14d0
      real*8, parameter :: Q_PREAL = 10.39d0 
      integer, parameter :: mdays_local = 31   ! March always has 31 days
     ...
     &       drydep_nh4_d(l), drydep_no3_d(l),l=1, matmo)
	    ...
	    !! write daily and/or monthly output
          ...
          if (isol == 1) call soil_write
           ...
           !write(lup_log_unit,*) 'debug', 'Year ', iyr, ' Month ', i_mo, "Day", idaf
           !write(lup_log_unit,*) "day sim=" , iida
           !flush(lup_log_unit) 
          
           !! === INSERTED: Dynamic LUP Change ===
           !write(soclup_log_unit,*) '--- Socio-Economic LUP (Reservoir) ---'
           
           !if (rchmono(2, SUB_TO_CHECK) > 0.) then
           !           Qs_march_avg = rchmono(2, SUB_TO_CHECK)
           !       else
           !           Qs_march_avg = 0.d0
           !           
           !       write(soclup_log_unit,*) 'Mean March flow [Subbasin 2] = ', Qs_march_avg, ' m3/s'
           !       endif
                  
           if ( curyr > nyskip .and. i_mo == 3 .and. iida == (91 - leapyr) ) then
               
               
               !! --- SOCIO-ECONOMIC LUP CHECK (once each March 31) ---
               if (USE_FLOW_TRIGGER == 0) then
                  !! === Reservoir-based rule ===
                  write(soclup_log_unit,*) '--- Socio-Economic LUP (Reservoir) ---'
                  write(soclup_log_unit,*) 'Year = ', iyr
                  write(soclup_log_unit,*) 'March 31 RES1 Volume = ', res_vol(1)/1.0e6, ' MCM'

                  if (res_vol(1) > 0.) then
                      res_vol_mcm = res_vol(1) / 1.0e6
                      if (res_vol_mcm < 312.0) then
                          if (res_vol_mcm > 246.0) then
                              selected_lup = 1
                          elseif (res_vol_mcm > 180.0) then
                              selected_lup = 2
                          else
                              selected_lup = 3
                          endif
                          current_lup_id = selected_lup
                          call resetsoclu
                          write(soclup_log_unit,*) 'Activated LUP Scenario ID = ', selected_lup
                      else
                          write(soclup_log_unit,*) 'Reservoir volume high → stay baseline.'
                      endif
                      flush(soclup_log_unit)
                  endif

               else
                  !! === Streamflow-based rule ===
                  ! Uses March mean flow from rchmon (rchmono(2, SUB_TO_CHECK))
                  if (rchmono(2, SUB_TO_CHECK) > 0.) then
                      !Qs_march_sum = rchmono(2, SUB_TO_CHECK)
                      !write(soclup_log_unit,*) 'sum March flow [Subbasin 2] = ', Qs_march_sum, ' m3/s'
                      Qs_march_avg = rchmono(2, SUB_TO_CHECK)/ (mdays_local)
                      !write(soclup_log_unit,*) 'Mean March flow [Subbasin 2] = ', Qs_march_avg, ' m3/s'
                  else
                      Qs_march_avg = 0.d0
                  endif

                  write(soclup_log_unit,*) '--- Socio-Economic LUP (Streamflow) ---'
                  write(soclup_log_unit,*) 'Year = ', iyr
                  write(soclup_log_unit,*) 'Mean March flow [Subbasin 2] = ', Qs_march_avg, ' m3/s'

                  desired_lup = 0
                  if (Qs_march_avg > Q_PREAL) then
                     write(soclup_log_unit,*) 'High flow. Remaining in baseline scenario.'
                     flush(soclup_log_unit)
                  elseif (Qs_march_avg > Q_ALERT) then
                     desired_lup = 1
                  elseif (Qs_march_avg > Q_EMERG) then
                     desired_lup = 2
                  else
                     desired_lup = 3
                  endif

                  if (desired_lup > 0 .and. desired_lup /= current_lup_id) then
                     selected_lup = desired_lup
                     current_lup_id = selected_lup
                     call resetsoclu
                     write(soclup_log_unit,*) 'Activated LUP Scenario ID = ', selected_lup
                  else
                     write(soclup_log_unit,*) 'No scenario change required.'
                  endif
                  flush(soclup_log_unit)
               endif
            endif
